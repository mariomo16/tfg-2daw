...
export class TimeSlotService {
  ...
  getAll(): Observable<TimeSlotExtended[]> {
    return this.#http
      .get<TimeSlotResponse[]>(this.#baseUrl)
      .pipe(map((timeslots) => timeslots.map(mapToTimeSlot)));
  }

  getById(id: number): Observable<TimeSlotExtended> {
    return this.#http
      .get<TimeSlotResponse>(`${this.#adminUrl}/${id}`)
      .pipe(map((timeslot) => mapToTimeSlot(timeslot)));
  }

  create(data: CreateTimeSlotDto): Observable<TimeSlotExtended> {
    return this.#http
      .post<TimeSlotResponse>(this.#adminUrl, data)
      .pipe(map((timeslot) => mapToTimeSlot(timeslot)));
  }

  update(id: number, data: UpdateTimeSlotDto): Observable<TimeSlotExtended> {
    return this.#http
